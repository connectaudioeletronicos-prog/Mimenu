  ...
  if (tipo === 'caixa') {
    ...
    listaPagamentos.innerHTML = (pagamentos_hoje || []).length === 0
      ...
      : pagamentos_hoje.map(c => `
        <div class="comanda-movimentacao">
          <div class="comanda-movimentacao__cabecalho">
            <span class="comanda-movimentacao__mesa">${c.origem === 'pedido' ? '🧾' : '🍽️'} ${escaparHtmlAdmin(c.mesa_cliente)}${c.origem === 'comanda' ? ` <span style="font-weight:600;color:var(--admin-cor-texto-claro);">— atendido por ${escaparHtmlAdmin(c.funcionario_nome || '-')}</span>` : ' <span style="font-weight:600;color:var(--admin-cor-texto-claro);">— venda direta no Atendimento</span>'}</span>
            <span class="comanda-movimentacao__status comanda-movimentacao__status--fechada">${formatarHoraAdmin(c.quando || c.fechada_em)}</span>
          </div>
          <div class="comanda-movimentacao__rodape">
