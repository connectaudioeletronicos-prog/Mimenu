...
async function calcularResumoPlantao(plantaoId, funcionarioId) {
  ...
  // Comissao = valor fixo por entrega + (valor por km * km RODADO ALEM DO
  // LIMITE incluso no fixo). Os 3 campos sao independentes:
  //   - So' "valor por km" preenchido (fixo=0, limite=0) -> comissao = so' km * valor_km, desde o km 1
  //   - So' "valor fixo" preenchido (km=0) -> comissao = so' o fixo, nao importa a distancia
  //   - Os dois preenchidos SEM limite (limite=0) -> soma direta: fixo + km*valor_km
  //   - Os dois preenchidos COM limite -> fixo cobre ate X km; alem disso,
  //     soma valor_km so' pelos km que passarem do limite
  //     (ex: fixo R$5 ate 4km, rota de 6km = R$5 + (6-4)*R$2 = R$9)
  const calcularComissaoPedido = (distanciaKm) => {
    const km = Number(distanciaKm) || 0;
    const kmExcedente = Math.max(0, km - (Number(f.km_incluido_no_fixo) || 0));
...
// Marca todos os plantoes fechados e ainda nao pagos de um entregador como
...
async function marcarPlantoesComoPago(req, res) {
  try {
    const { id } = req.params;
    const alvo = await query('SELECT id FROM funcionarios WHERE id = $1 AND estabelecimento_id = $2 AND cargo = $3', [id, req.estabelecimentoId, 'entregador']);
    if (alvo.rows.length === 0) return res.status(404).json({ erro: 'Entregador nao encontrado.' });

    const resultado = await query(
      `UPDATE entregador.plantoes_entregador SET pago = true
       WHERE funcionario_id = $1 AND fim IS NOT NULL AND pago = false
       RETURNING id`,
      [id]
    );
    res.json({ mensagem: `${resultado.rows.length} plantao(s) marcado(s) como pago.`, quantidade: resultado.rows.length });
  } catch (error) {
    console.error('Erro ao marcar plantoes como pagos:', error);
    res.status(500).json({ erro: 'Erro ao marcar plantoes como pagos.' });
  }
}
