...
// --- Mascara de telefone com hifen, adaptativa (fixo 4+4 ou celular 5+4) ---
// Usada em telas de "meus dados" onde o numero e exibido formatado, ao
// contrario do campo de cadastro (que usa o formato sem hifen exigido
// pelo backend).
function aplicarMascaraTelefoneComHifen(campo) {
  if (!campo || campo.dataset.mascara) return;
  campo.dataset.mascara = '1';
  campo.addEventListener('input', function () {
    let numeros = this.value.replace(/\D/g, '').substring(0, 11);
    let valor = '';
    if (numeros.length > 0) valor = '(' + numeros.substring(0, 2);
    if (numeros.length >= 2) valor += ') ';
    if (numeros.length > 2) {
      if (numeros.length <= 10) {
        valor += numeros.substring(2, 6);
        if (numeros.length > 6) valor += '-' + numeros.substring(6, 10);
      } else {
        valor += numeros.substring(2, 7);
        if (numeros.length > 7) valor += '-' + numeros.substring(7, 11);
      }
    }
    this.value = valor;
  });
...
function aplicarMascaraCnpj(campo) {
  ...
  campo.dataset.mascara = '1';
  campo.addEventListener('input', function () {
    let numeros = this.value.replace(/\D/g, '').substring(0, 14);
    let valor = numeros;
    if (numeros.length > 12) valor = `${numeros.slice(0,2)}.${numeros.slice(2,5)}.${numeros.slice(5,8)}/${numeros.slice(8,12)}-${numeros.slice(12)}`;
    else if (numeros.length > 8) valor = `${numeros.slice(0,2)}.${numeros.slice(2,5)}.${numeros.slice(5,8)}/${numeros.slice(8)}`;
    else if (numeros.length > 5) valor = `${numeros.slice(0,2)}.${numeros.slice(2,5)}.${numeros.slice(5)}`;
    else if (numeros.length > 2) valor = `${numeros.slice(0,2)}.${numeros.slice(2)}`;
    this.value = valor;
  });
}
