...
let FILTRO_FORMA_PAGAMENTO_ENTREGADOR = null; // 'km' | 'entrega' | null -- aplicado na vista "Equipe de entregadores"
...
async function salvarValorEntregaGlobal() {
  const valor = document.getElementById('valor-entrega-global-input').value;
  if (valor === '' || Number(valor) < 0) { mostrarToast('Digite um valor válido.', true); return; }
  ...
  try {
    ...
    await Promise.all(entregadores.map(f => apiAtualizarFuncionario(f.id, {
      forma_pagamento_entrega: MODO_VALOR_ENTREGA_GLOBAL,
      [campoValor]: valor
    })));
...
function mostrarVistaEquipeEntregas(filtroFormaPagamento = null) {
  FILTRO_FORMA_PAGAMENTO_ENTREGADOR = filtroFormaPagamento;
  ocultarTodasVistasFuncionarios();
  document.getElementById('funcionarios-vista-entregas-equipe')?.classList.remove('oculto');
  configurarMenuLateralParaEntregas(true);

  const elFiltro = document.getElementById('central-entregas-filtro-ativo');
  if (filtroFormaPagamento) {
    const rotulo = filtroFormaPagamento === 'km' ? 'Por KM' : 'Valor Fixo';
    elFiltro.classList.remove('oculto');
    elFiltro.innerHTML = `<span class="badge-fila">Mostrando: ${rotulo}</span> <button type="button" id="botao-limpar-filtro-entregador" class="botao-secundario" style="padding:4px 10px;font-size:0.75rem;">Ver todos</button>`;
    document.getElementById('botao-limpar-filtro-entregador').addEventListener('click', () => mostrarVistaEquipeEntregas(null));
  } else {
    elFiltro.classList.add('oculto');
    elFiltro.innerHTML = '';
  }

  if (ULTIMA_EQUIPE_OPERACIONAL) renderizarListaEquipe('lista-equipe-entregadores', ULTIMA_EQUIPE_OPERACIONAL.entregadores, 'entregador');
}
...
function renderizarListaEquipe(idLista, itens, tipo) {
  ...
  // Quando a "Equipe de entregadores" e' aberta a partir de um dos botoes
  // "Por KM" / "Valor Fixo" na Central de Entregas, mostra so quem usa
  // aquela forma de pagamento (FILTRO_FORMA_PAGAMENTO_ENTREGADOR).
  if (tipo === 'entregador' && FILTRO_FORMA_PAGAMENTO_ENTREGADOR) {
    const alvo = FILTRO_FORMA_PAGAMENTO_ENTREGADOR;
    itens = (itens || []).filter(f => (alvo === 'km' ? f.forma_pagamento_entrega === 'km' : f.forma_pagamento_entrega !== 'km'));
  }
    ...
    if (tipo === 'entregador') {
      ...
      extra = `
        ...
        <div style="margin-top:8px; display:flex; gap:8px; flex-wrap:wrap;">
          ...
          <button type="button" class="botao-secundario" data-ver-valores-entregador="${f.id}" data-nome-entregador="${escaparHtmlAdmin(f.nome)}">💰 Histórico e valores</button>
        </div>
      `;
    }
    return `
  ...
  lista.querySelectorAll('[data-ver-valores-entregador]').forEach(botao => {
    botao.addEventListener('click', () => {
      abrirDetalheEntregador(botao.getAttribute('data-ver-valores-entregador'), botao.getAttribute('data-nome-entregador'));
    });
  });
}
...
function configurarFuncionarios() {
  ...
  document.getElementById('botao-abrir-equipe-entregas')?.addEventListener('click', () => mostrarVistaEquipeEntregas(null));
