...
async function carregarDadosCentralEntregas() {
  ...
  try {
    const pedidos = (await apiListarPedidos()).filter(p => p.tipo_pedido !== 'balcao');
    const hojeStr = new Date().toDateString();
    const ehHoje = (iso) => iso && new Date(iso).toDateString() === hojeStr;

    const emRota = pedidos.filter(p => p.status_pedido === 'saiu_entrega');
    const aguardandoColeta = pedidos.filter(p => p.status_pedido === 'pronto');
    const finalizadasHoje = pedidos.filter(p => p.status_pedido === 'entregue' && ehHoje(p.atualizado_em));
    const entregasHoje = pedidos.filter(p => ehHoje(p.criado_em)).length;

    document.getElementById('stat-entregas-hoje').textContent = entregasHoje;
    document.getElementById('stat-em-andamento').textContent = emRota.length;
    ...
    const linhas = [...emRota, ...aguardandoColeta];
    if (linhas.length === 0) {
      listaEl.innerHTML = '<div class="lista-vazia">Nenhuma entrega em andamento agora.</div>';
      return;
    }

    listaEl.innerHTML = linhas.map(p => {
      const emRotaAgora = p.status_pedido === 'saiu_entrega';
      const iniciais = (p.entregador_nome || '?').trim().charAt(0).toUpperCase();
      const hora = new Date(emRotaAgora ? (p.horario_saiu_entrega || p.atualizado_em) : p.criado_em)
        .toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' });
      const clicavel = p.entregador_id ? `data-abrir-detalhe-entregador="${p.entregador_id}" data-nome-entregador-linha="${escaparHtmlAdmin(p.entregador_nome)}" role="button" tabindex="0"` : '';
      return `
        <div class="entrega-linha ${p.entregador_id ? 'entrega-linha--clicavel' : ''}" ${clicavel}>
          <div class="entrega-linha__avatar" style="background:${corAvatarEntregador(p.entregador_id || p.entregador_nome)}">${iniciais}</div>
          <div class="entrega-linha__info">
            <div class="entrega-linha__titulo">${escaparHtmlAdmin(p.entregador_nome || 'Aguardando entregador')} · #${p.numero_pedido || p.id.slice(0, 6)}</div>
            <div class="entrega-linha__subtitulo">${escaparHtmlAdmin(p.cliente_nome)} · ${escaparHtmlAdmin(p.cliente_endereco || 'Endereço não informado')}</div>
          </div>
          <div style="text-align:right;">
            <span class="entrega-linha__badge ${emRotaAgora ? 'entrega-linha__badge--rota' : 'entrega-linha__badge--aguardando'}">${emRotaAgora ? 'Em rota' : 'Aguardando'}</span>
            <div class="entrega-linha__subtitulo" style="margin-top:4px;">${hora}</div>
          </div>
        </div>
      `;
    }).join('');
