...
async function salvarValorEntregaGlobal() {
  ...
  try {
    ...
    await Promise.all(entregadores.map(f => apiAtualizarFuncionario(f.id, { [campoValor]: valor })));
...
async function abrirDetalheEntregador(id, nomeConhecido = null) {
  ...
  document.getElementById('detalhe-entregador-situacao').textContent = funcionario
    ? [
        Number(funcionario.valor_por_entrega) > 0 ? `Fixo: R$ ${formatarMoedaAdmin(funcionario.valor_por_entrega)}` : null,
        Number(funcionario.valor_por_km) > 0 ? `R$ ${formatarMoedaAdmin(funcionario.valor_por_km)}/km` : null
