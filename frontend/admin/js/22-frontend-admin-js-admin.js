...
async function abrirDetalheEntregador(id, nomeConhecido = null) {
  ...
  try {
    const plantoes = await apiHistoricoPlantoesEntregador(id);
    renderizarDetalheEntregador(plantoes);
  } catch (erro) {
    document.getElementById('detalhe-entregador-historico').innerHTML = '<div class="lista-vazia">Não foi possível carregar o histórico.</div>';
  }
}
...
function configurarFuncionarios() {
  ...
  document.querySelectorAll('[data-filtro-forma-pagamento]').forEach(botao => {
    botao.addEventListener('click', () => mostrarVistaEquipeEntregas(botao.getAttribute('data-filtro-forma-pagamento')));
  });
