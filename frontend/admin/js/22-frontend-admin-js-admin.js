...
function abrirPainelValorEntregaGlobal(modo) {
  ...
  const ehHibrido = modo === 'hibrido';
  document.getElementById('valor-entrega-global-titulo').textContent =
    modo === 'km' ? 'Valor por KM' : modo === 'entrega' ? 'Valor fixo por entrega' : 'Modelo híbrido (fixo + km excedente)';

  document.getElementById('valor-entrega-global-campo-simples').classList.toggle('oculto', ehHibrido);
  document.getElementById('valor-entrega-global-campo-hibrido').classList.toggle('oculto', !ehHibrido);

  // Pre-preenche com o valor ja salvo (do primeiro entregador cadastrado
  // que usa esse modo) em vez de sempre abrir vazio -- antes isso parecia
  // que o valor salvo tinha sumido, quando na verdade so nao aparecia.
  const entregadorComValor = (ULTIMA_EQUIPE_OPERACIONAL?.entregadores || [])
    .find(f => f.forma_pagamento_entrega === modo);

  if (ehHibrido) {
    document.getElementById('valor-hibrido-fixo-input').value = entregadorComValor ? Number(entregadorComValor.valor_por_entrega || 0).toFixed(2).replace('.', ',') : '';
    document.getElementById('valor-hibrido-km-incluido-input').value = entregadorComValor ? String(entregadorComValor.km_incluido_no_fixo ?? 0).replace('.', ',') : '';
    document.getElementById('valor-hibrido-km-excedente-input').value = entregadorComValor ? Number(entregadorComValor.valor_por_km || 0).toFixed(2).replace('.', ',') : '';
    atualizarExemploHibrido();
  } else {
...
// Enquanto digita, so deixa passar numero e virgula (formato brasileiro) --
// os campos sao texto (nao number) porque o teclado numerico do celular no
// tipo "number" as vezes nao permite digitar virgula, so ponto.
function permitirSoNumeroEVirgula(evento) {
  evento.target.value = evento.target.value.replace(/[^0-9,]/g, '').replace(/(,.*),/g, '$1');
}
['valor-entrega-global-input', 'valor-hibrido-fixo-input', 'valor-hibrido-km-incluido-input', 'valor-hibrido-km-excedente-input'].forEach(id => {
  document.getElementById(id)?.addEventListener('input', permitirSoNumeroEVirgula);
});
['valor-hibrido-fixo-input', 'valor-hibrido-km-incluido-input', 'valor-hibrido-km-excedente-input'].forEach(id => {
  document.getElementById(id)?.addEventListener('input', atualizarExemploHibrido);
});

// Mostra um exemplo concreto de calculo enquanto o lojista digita, pra
// confirmar visualmente que o modelo hibrido esta configurado do jeito
// que ele espera antes de salvar (evita confusao/erro de calculo).
function atualizarExemploHibrido() {
  const fixo = Number((document.getElementById('valor-hibrido-fixo-input').value || '0').replace(',', '.')) || 0;
  const kmIncluido = Number((document.getElementById('valor-hibrido-km-incluido-input').value || '0').replace(',', '.')) || 0;
  const kmExcedente = Number((document.getElementById('valor-hibrido-km-excedente-input').value || '0').replace(',', '.')) || 0;
  const exemploKmRota = kmIncluido + 3; // so pra ilustrar: uma rota 3km alem do incluso
  const kmQueSomam = Math.max(0, exemploKmRota - kmIncluido);
  const total = fixo + kmQueSomam * kmExcedente;
  document.getElementById('valor-hibrido-exemplo').textContent =
    `Exemplo: uma rota de ${exemploKmRota.toLocaleString('pt-BR')} km ficaria em R$ ${fixo.toFixed(2).replace('.', ',')} (fixo) + ${kmQueSomam.toLocaleString('pt-BR')} km × R$ ${kmExcedente.toFixed(2).replace('.', ',')} = R$ ${total.toFixed(2).replace('.', ',')} no total.`;
}
...
async function salvarValorEntregaGlobal() {
  ...
  if (MODO_VALOR_ENTREGA_GLOBAL === 'hibrido') {
    const fixo = document.getElementById('valor-hibrido-fixo-input').value.replace(',', '.');
    const kmIncluido = document.getElementById('valor-hibrido-km-incluido-input').value.replace(',', '.');
    const excedente = document.getElementById('valor-hibrido-km-excedente-input').value.replace(',', '.');
    if ([fixo, kmIncluido, excedente].some(v => v === '' || isNaN(Number(v)) || Number(v) < 0)) {
      mostrarToast('Preencha os 3 valores corretamente.', true);
      return;
    }
    dadosParaSalvar = { forma_pagamento_entrega: 'hibrido', valor_por_entrega: fixo, km_incluido_no_fixo: kmIncluido, valor_por_km: excedente };
  } else {
    ...
    dadosParaSalvar = { forma_pagamento_entrega: MODO_VALOR_ENTREGA_GLOBAL, [campoValor]: valor };
  }
...
async function abrirDetalheEntregador(id, nomeConhecido = null) {
  ...
  document.getElementById('detalhe-entregador-situacao').textContent = funcionario
    ? (funcionario.forma_pagamento_entrega === 'km'
        ? `Comissão por KM: R$ ${formatarMoedaAdmin(funcionario.valor_por_km || 0)}/km`
        : `Comissão por entrega: R$ ${formatarMoedaAdmin(funcionario.valor_por_entrega || 0)}`)
    : '';
