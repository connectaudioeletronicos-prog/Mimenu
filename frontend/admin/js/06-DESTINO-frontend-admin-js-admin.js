...
function configurarMenu() {
  ...
  document.querySelectorAll('.painel__menu-item[data-aba]').forEach(botao => {
    botao.addEventListener('click', () => {
      ...
      // sessao pra sessao, so dentro da mesma navegacao).
      // O PROPRIETARIO tem acesso absoluto e nunca passa por esse gate --
      // ele nao e um registro em "funcionarios" (loga por fora dessa
      // tabela), entao a senha dele jamais seria aceita ali. Como ja esta
      // autenticado como dono da conta, entra direto.
      if (aba === 'atendimento' && sessaoAtual().cargo === 'proprietario' && !atendimentoAutenticado) {
        atendimentoAutenticado = { id: null, nome: sessaoAtual().nome || 'Proprietario', cargo: 'proprietario' };
        const cargoLegenda = { administrador: 'Administrador', gerente: 'Gerente', caixa: 'Caixa', proprietario: 'Proprietario' };
        const rotulo = document.getElementById('atendimento-autenticado-como');
        if (rotulo) rotulo.textContent = `👤 ${atendimentoAutenticado.nome} (${cargoLegenda[atendimentoAutenticado.cargo]})`;
        ativarAba(botao);
        return;
      }
      ...
      : abertas.map(c => `
        <div class="comanda-movimentacao">
          ...
          <div class="comanda-movimentacao__rodape">
            ...
            <button type="button" class="comanda-movimentacao__corrigir" data-cobrar-comanda-garcom="${c.id}" data-total="${c.subtotal}">💳 Cobrar</button>
          </div>
        </div>
      `).join('');
    container.querySelectorAll('[data-cobrar-comanda-garcom]').forEach(botao => {
      botao.addEventListener('click', () => abrirCobrancaComandaGarcom(botao.dataset.cobrarComandaGarcom, parseFloat(botao.dataset.total)));
    });
  } catch (erro) {
...
function abrirCobrancaComandaGarcom(comandaId, subtotal) {
  const forma = prompt('Forma de pagamento: dinheiro, pix, cartao_credito ou cartao_debito');
  const formasValidas = ['dinheiro', 'pix', 'cartao_credito', 'cartao_debito'];
  if (!forma || !formasValidas.includes(forma.trim())) { if (forma !== null) mostrarToast('Forma de pagamento inválida.', true); return; }
  const gorjetaTexto = prompt('Gorjeta (R$) -- deixe em branco se não houver', '0');
  const gorjeta = parseFloat((gorjetaTexto || '0').replace(',', '.')) || 0;
  apiFecharComandaNoCaixa(comandaId, { forma_pagamento: forma.trim(), gorjeta })
    .then(() => {
      mostrarToast(`Comanda cobrada — R$ ${formatarMoedaAdmin(subtotal + gorjeta)}`);
      abrirPainelComandaGarcom();
    })
...
function construirAcoesStatusPedido(pedido) {
  ...
  // Atribuicao manual a um entregador especifico -- diferente do botao
  // "Confirmar pronto" (que usa a fila automatica), isso deixa o gestor
  // escolher quem recebe, inclusive um entregador que ja esta com outra
  // entrega em andamento (forma uma rota com varias paradas pra ele).
  if (pedido.tipo_pedido === 'entrega' && pedido.status_pedido === 'pronto' && temPermissao('mudar_status_pedidos')) {
    const entregadores = (ESTADO.funcionarios || []).filter(f => f.cargo === 'entregador' && f.ativo);
    if (entregadores.length > 0) {
      const opcoes = entregadores.map(f => `<option value="${f.id}">${escaparHtmlAdmin(f.nome)}</option>`).join('');
      html += `
        <div class="atribuir-entregador-manual" style="margin-top:8px; display:flex; gap:6px;">
          <select data-select-entregador="${pedido.id}" style="flex:1;">
            <option value="">Enviar para um entregador especifico...</option>
            ${opcoes}
          </select>
          <button type="button" class="botao-secundario" data-atribuir-entregador="${pedido.id}">Enviar</button>
        </div>`;
    }
  }
...
function renderizarPedidosAdmin(pedidos) {
  ...
  lista.querySelectorAll('[data-atribuir-entregador]').forEach(botao => {
    botao.addEventListener('click', async () => {
      const id = botao.getAttribute('data-atribuir-entregador');
      const select = lista.querySelector(`[data-select-entregador="${id}"]`);
      const entregadorId = select ? select.value : '';
      if (!entregadorId) {
        mostrarToast('Escolha um entregador na lista.', true);
        return;
      }
      try {
        await apiAtribuirEntregador(id, entregadorId);
        mostrarToast('Pedido enviado para o entregador escolhido.');
        carregarPedidos();
      } catch (erro) {
        mostrarToast(erro.message, true);
      }
...
async function carregarReservas() {
  ...
  try {
    ...
    const rotulosStatus = { pendente: 'Pendente', confirmada: 'Confirmada', cancelada: 'Cancelada' };
      ...
      <div class="item-admin">
        <div class="item-admin__info">
          ...
          <div class="item-admin__subtitulo">
            ${new Date(r.data_reserva).toLocaleDateString('pt-BR', { timeZone: 'UTC' })} às ${r.horario_reserva} · ${r.cliente_telefone} · ${rotulosStatus[r.status] || r.status}
          </div>
        </div>
        <div style="display:flex; gap:6px;">
          ${r.status === 'pendente' ? `<button type="button" class="botao-secundario" data-reserva-status="${r.id}:confirmada">Confirmar</button>` : ''}
          ${r.status !== 'cancelada' ? `<button type="button" class="botao-secundario" data-reserva-status="${r.id}:cancelada">Cancelar</button>` : ''}
        </div>
      </div>
    `).join('');
...
function renderizarResumoFuncionario(dados) {
  ...
  if (tipo === 'caixa') {
    // Caixa nunca "abre" mesa -- so mostra o resumo de pagamentos que ele
    // recebeu hoje, e deixa so a aba "Histórico" disponível (que já busca
    // por fechada_por_funcionario_id no backend pra esse cargo).
    ...
    document.getElementById('tela-resumo-caixa').style.display = 'none';
    document.querySelectorAll('.resumo-funcionario__aba-btn[data-tela-resumo="pedidos"], .resumo-funcionario__aba-btn[data-tela-resumo="caixa"]').forEach(b => b.classList.add('oculto'));
    document.querySelector('.resumo-funcionario__alternador').classList.remove('oculto');

    const { resumo, pagamentos_hoje } = dados;
    ...
    const listaPagamentos = document.getElementById('lista-caixa-pagamentos-hoje');
    listaPagamentos.innerHTML = (pagamentos_hoje || []).length === 0
      ? '<p class="aba__descricao">Nenhum pagamento recebido no caixa nesse período.</p>'
      : pagamentos_hoje.map(c => `
        <div class="comanda-movimentacao">
          <div class="comanda-movimentacao__cabecalho">
            <span class="comanda-movimentacao__mesa">${escaparHtmlAdmin(c.mesa_cliente)} <span style="font-weight:600;color:var(--admin-cor-texto-claro);">— atendido por ${escaparHtmlAdmin(c.funcionario_nome || '-')}</span></span>
            <span class="comanda-movimentacao__status comanda-movimentacao__status--fechada">${formatarHoraAdmin(c.fechada_em)}</span>
          </div>
          <div class="comanda-movimentacao__rodape">
            <span>${formasLegenda[c.forma_pagamento] || c.forma_pagamento || '-'}</span>
            <span class="comanda-movimentacao__total">R$ ${formatarMoedaAdmin(c.total)}</span>
          </div>
        </div>
      `).join('');

    telaResumoAtiva = 'historico';
  ...
  if (!comandas_hoje || comandas_hoje.length === 0) {
    container.innerHTML = '<p class="aba__descricao">Nenhuma movimentação nesse período.</p>';
  } else {
    container.innerHTML = comandas_hoje.map(c => `
      <div class="comanda-movimentacao">
        <div class="comanda-movimentacao__cabecalho">
          <span class="comanda-movimentacao__mesa">${escaparHtmlAdmin(c.mesa_cliente)}</span>
          <span class="comanda-movimentacao__status comanda-movimentacao__status--${c.status}">${c.status === 'aberta' ? 'Aberta' : 'Finalizada'}</span>
        </div>
        ${(c.rodadas || []).flatMap(r => Array.isArray(r.itens) ? r.itens : []).map(item => `
        ...
        `).join('')}
        <div class="comanda-movimentacao__rodape">
          <span>${c.status === 'fechada' ? (formasLegenda[c.forma_pagamento] || c.forma_pagamento) : 'Aguardando fechamento'}</span>
          <span class="comanda-movimentacao__total">R$ ${formatarMoedaAdmin(c.total)}</span>
          <button type="button" class="comanda-movimentacao__corrigir" data-corrigir-comanda="${c.id}" data-mesa="${escaparHtmlAdmin(c.mesa_cliente)}" data-subtotal="${c.subtotal}" data-gorjeta="${c.gorjeta}">✏️ Corrigir</button>
        </div>
      </div>
    `).join('');
...
async function carregarHistoricoFuncionario(funcionarioId, pagina, acrescentar) {
  ...
  try {
    ...
    if (comandas.length === 0 && !acrescentar) {
      container.innerHTML = '<p class="aba__descricao">Esse funcionário ainda não tem nenhuma comanda fechada no histórico.</p>';
      return;
    }

    const formasLegenda = { dinheiro: 'Dinheiro', pix: 'PIX', cartao_credito: 'Cartão Crédito', cartao_debito: 'Cartão Débito' };
    const listaHtml = comandas.map(c => `
      <div class="comanda-movimentacao">
        <div class="comanda-movimentacao__cabecalho">
