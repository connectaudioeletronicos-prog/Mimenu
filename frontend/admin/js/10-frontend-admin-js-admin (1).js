...
function renderizarIdentificacaoEntregadores(entregadores) {
  ...
  lista.innerHTML = entregadores.map(f => {
    const situacao = !f.ativo ? 'Inativo'
      : f.em_entrega ? 'Em entrega'
      : !f.disponivel_entrega ? 'Indisponível'
      : `Fila #${f.posicao_fila}`;
    return `
      <div class="item-admin">
        <div class="item-admin__info">
          <div class="item-admin__titulo">${escaparHtmlAdmin(f.nome)}</div>
          <div class="item-admin__subtitulo"><span class="badge-fila">${situacao}</span></div>
        </div>
      </div>
    `;
  }).join('');
