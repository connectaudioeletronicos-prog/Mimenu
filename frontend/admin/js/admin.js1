...
function aplicarVisibilidadeMenu() {
  ...
  // Estoque so aparece se o lojista tiver ligado o modulo em Configuracoes.
  const estoqueAtivo = ESTADO.estabelecimento && ESTADO.estabelecimento.estoque_modulo_ativo;
  document.getElementById('menu-item-estoque')?.classList.toggle('oculto', !(estoqueAtivo && temPermissao('gerenciar_estoque')));
}
