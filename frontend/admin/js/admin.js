...
function atualizarContadoresMenu() {
  ...
  // Badge de reservas pendentes -- avisa direto no menu que tem gente
  // esperando confirmacao, sem precisar entrar na aba pra descobrir
  // (antes disso reservas nao davam nenhum aviso visivel no painel).
  if (document.getElementById('menu-item-reservas') && !document.getElementById('menu-item-reservas').classList.contains('oculto') && typeof apiListarReservas === 'function') {
    apiListarReservas()
      .then(reservas => {
        const pendentes = (reservas || []).filter(r => r.status === 'pendente').length;
        definir('[data-menu-contador="reservas"]', pendentes);
      })
      .catch(() => {});
  }
...
function preencherFormularios() {
  ...
  if (e) {
    ...
    // Cabecalho da pagina "Informacoes da loja": logo (o Logotipo, o mesmo
    // exibido ao cliente no cardapio), nome e slug da loja.
    const infoLogo = document.getElementById('informacoes-logo');
    const infoNome = document.getElementById('informacoes-nome-loja');
    const infoSlug = document.getElementById('informacoes-slug');
    const infoEmailLogin = document.getElementById('campo-email-login');
    if (infoLogo && infoNome && infoSlug) {
      if (e.logo_apps_url) {
        infoLogo.src = e.logo_apps_url;
        infoLogo.classList.remove('oculto');
      } else {
        infoLogo.classList.add('oculto');
      }
      infoNome.textContent = e.nome || 'Minha loja';
      infoSlug.textContent = e.slug ? `/${e.slug}` : '';
    }
    if (infoEmailLogin) infoEmailLogin.value = e.email || '';
...
async function carregarReservas() {
  ...
  try {
    ...
    const rotulosStatus = { pendente: 'Pendente', confirmada: 'Confirmada', concluida: 'Concluída', nao_concluida: 'Não compareceu' };
    ...
    lista.innerHTML = reservas.map(r => `
      <div class="item-admin item-admin--clicavel" data-reserva-detalhe="${r.id}">
        <div class="item-admin__info">
          ...
          <div class="item-admin__subtitulo">
            ${new Date(r.data_reserva).toLocaleDateString('pt-BR', { timeZone: 'UTC' })} às ${r.horario_reserva} · ${r.cliente_telefone} · <span class="reserva-status reserva-status--${r.status}">${rotuloStatusReserva(r)}</span>
          </div>
          ${r.observacoes ? `<div class="item-admin__subtitulo" style="opacity:0.8;">💬 ${escaparHtmlAdmin(r.observacoes)}</div>` : ''}
        </div>
        <div style="display:flex; gap:6px; flex-wrap:wrap;" onclick="event.stopPropagation()">
          ...
          ${r.status === 'confirmada' ? `<button type="button" class="botao-primario" data-reserva-checkin="${r.id}">✅ Check-in</button>` : ''}
          ${['pendente', 'confirmada'].includes(r.status) ? `<button type="button" class="botao-secundario" data-reserva-status="${r.id}:cancelada">Cancelar</button>` : ''}
        </div>
      </div>
    `).join('');

    // Clicar em qualquer lugar do card (fora dos botoes de acao, que
    // param a propagacao acima) abre o detalhe completo: nome, telefone,
    // data/hora no padrao, observacoes do cliente, e a linha do tempo
    // (solicitada em / confirmada-recusada em / concluida-nao compareceu
    // em) -- inclusive pra reservas ja no historico.
    lista.querySelectorAll('[data-reserva-detalhe]').forEach(card => {
      card.addEventListener('click', () => {
        const id = card.getAttribute('data-reserva-detalhe');
        const reserva = reservas.find(r => r.id === id);
        if (reserva) abrirDetalheReserva(reserva, rotuloStatusReserva(reserva));
      });
    });
    ...
    lista.querySelectorAll('[data-reserva-checkin]').forEach(botao => {
      botao.addEventListener('click', () => {
        const id = botao.getAttribute('data-reserva-checkin');
        const reserva = reservas.find(r => r.id === id);
        if (reserva) abrirModalCheckIn(reserva);
      });
    });
  } catch (erro) {
...
// Check-in de chegada exige confirmar o telefone do cliente antes de
// efetivar -- nao e' um clique unico direto, pra evitar dar entrada na
// reserva errada por engano. So depois de confirmar corretamente e' que
// a reserva vira "concluida" e aparece pro cliente e no historico.
function abrirModalCheckIn(reserva) {
  document.getElementById('checkin-reserva-nome').textContent = reserva.cliente_nome;
  document.getElementById('checkin-reserva-info').textContent =
    `${new Date(reserva.data_reserva).toLocaleDateString('pt-BR', { timeZone: 'UTC' })} às ${String(reserva.horario_reserva).substring(0, 5)} · ${reserva.quantidade_pessoas} pessoa(s)`;
  const input = document.getElementById('checkin-reserva-telefone-input');
  const erroEl = document.getElementById('checkin-reserva-erro');
  input.value = '';
  erroEl.classList.add('oculto');

  const botaoConfirmar = document.getElementById('checkin-reserva-confirmar');
  const novoBotao = botaoConfirmar.cloneNode(true); // limpa listener anterior
  botaoConfirmar.replaceWith(novoBotao);

  novoBotao.addEventListener('click', async () => {
    const digitado = input.value.replace(/\D/g, '');
    const esperado = (reserva.cliente_telefone || '').replace(/\D/g, '');
    if (!digitado || digitado !== esperado) {
      erroEl.textContent = 'Telefone não confere com o da reserva.';
      erroEl.classList.remove('oculto');
      return;
    }
    novoBotao.disabled = true;
    try {
      await apiCheckInReserva(reserva.id);
      document.getElementById('modal-checkin-reserva').classList.add('oculto');
      mostrarToast('Check-in feito! Reserva concluída.');
      carregarReservas();
      atualizarContadoresMenu();
    } catch (erro) {
      erroEl.textContent = erro.message;
  ...
  document.getElementById('modal-checkin-reserva').classList.remove('oculto');
}
// data e hora no padrao dd/mm/aaaa aas HH:MM, observacoes que o cliente
// escreveu ao reservar, e a linha do tempo completa (quando foi
// solicitada, quando foi confirmada/recusada, quando teve check-in ou
// foi marcada como nao compareceu). Funciona igual pra reserva ativa ou
// ja no historico -- e' so' um raio-x da mesma reserva, os dados nao
// mudam dependendo de onde foi clicado.
function abrirDetalheReserva(r, rotuloStatus) {
  const dataReservaISO = String(r.data_reserva).substring(0, 10);
  const [ano, mes, dia] = dataReservaISO.split('-');
  const dataFormatada = `${dia}/${mes}/${ano}`;
  const horaFormatada = String(r.horario_reserva).substring(0, 5);

  const formatarDataHora = (iso) => iso ? new Date(iso).toLocaleString('pt-BR', { dateStyle: 'short', timeStyle: 'short' }) : null;

  const linhaTempo = [`📩 Solicitada em ${formatarDataHora(r.criado_em)}`];
  if (r.status === 'confirmada' && r.atualizado_em) {
    linhaTempo.push(`✅ Confirmada em ${formatarDataHora(r.atualizado_em)}`);
  } else if (r.status === 'concluida') {
    if (r.atualizado_em && r.atualizado_em !== r.criado_em) linhaTempo.push(`✅ Confirmada anteriormente`);
    linhaTempo.push(`🎉 Check-in feito em ${formatarDataHora(r.check_in_em || r.atualizado_em)}`);
  } else if (r.status === 'nao_concluida') {
    linhaTempo.push(`✕ Marcada como não compareceu em ${formatarDataHora(r.atualizado_em)}`);
  } else if (r.status === 'cancelada') {
    const quem = r.cancelada_por === 'cliente' ? 'pelo cliente' : 'pela loja';
    linhaTempo.push(`✕ Cancelada ${quem} em ${formatarDataHora(r.atualizado_em)}`);
  }

  document.getElementById('detalhe-reserva-titulo').textContent = r.cliente_nome;
  document.getElementById('detalhe-reserva-conteudo').innerHTML = `
    <p style="margin:0 0 10px;"><span class="reserva-status reserva-status--${r.status}">${rotuloStatus}</span></p>
    <p style="margin:4px 0;"><strong>📅 Data e hora:</strong> ${dataFormatada} às ${horaFormatada}</p>
    <p style="margin:4px 0;"><strong>👥 Pessoas:</strong> ${r.quantidade_pessoas}</p>
    <p style="margin:4px 0;"><strong>📞 Telefone:</strong> ${r.cliente_telefone}</p>
    ${r.observacoes ? `<p style="margin:4px 0;"><strong>💬 Observações do cliente:</strong><br>${escaparHtmlAdmin(r.observacoes)}</p>` : '<p style="margin:4px 0; opacity:0.7;">Sem observações do cliente.</p>'}
    <hr style="margin:14px 0; border:none; border-top:1px solid var(--admin-cor-borda);">
    <p style="margin:4px 0; font-size:0.85rem; opacity:0.85;">${linhaTempo.join('<br>')}</p>
  `;
  document.getElementById('modal-detalhe-reserva').classList.remove('oculto');
}
