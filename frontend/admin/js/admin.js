...
function renderizarResumoFuncionario(dados) {
  ...
  document.getElementById('marcador-gorjeta').classList.toggle('oculto', tipo === 'sem_dados');
  if (tipo === 'sem_dados') return;

  const { resumo, comandas_hoje, fechamento_caixa } = dados;
  document.getElementById('marcador-gorjeta').textContent = `🪙 R$ ${formatarMoedaAdmin(resumo.gorjetas_do_dia || 0)}`;
