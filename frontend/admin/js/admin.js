...
async function abrirPainelComandaGarcom() {
  ...
  modoComandaGarcom = 'aberta';
  atualizarBotoesModoComandaGarcom();
  await carregarListaComandaGarcom();
}

function atualizarBotoesModoComandaGarcom() {
  document.getElementById('botao-comanda-garcom-abertas').className = modoComandaGarcom === 'aberta' ? 'botao-primario' : 'botao-secundario';
  document.getElementById('botao-comanda-garcom-fechadas').className = modoComandaGarcom === 'fechada' ? 'botao-primario' : 'botao-secundario';
  document.getElementById('botao-comanda-garcom-abertas').style.flex = '1';
  document.getElementById('botao-comanda-garcom-fechadas').style.flex = '1';
}

document.getElementById('botao-comanda-garcom-abertas')?.addEventListener('click', () => {
  modoComandaGarcom = 'aberta';
  atualizarBotoesModoComandaGarcom();
  carregarListaComandaGarcom();
});
document.getElementById('botao-comanda-garcom-fechadas')?.addEventListener('click', () => {
  modoComandaGarcom = 'fechada';
  atualizarBotoesModoComandaGarcom();
  carregarListaComandaGarcom();
});
...
async function carregarListaComandaGarcom() {
  ...
  try {
    if (modoComandaGarcom === 'aberta') {
      ...
      // Fechadas hoje -- so consulta, nao pode editar nem cobrar de novo.
      // Serve pra checar rapido "quem atendeu / quem cobrou / quando" de
      // qualquer garcom, exatamente pra resolver duvida de cliente.
      const fechadas = await chamarApiAdmin('/comandas?status=fechada&limite=100');
      container.innerHTML = fechadas.length === 0
        ? '<p class="aba__descricao">Nenhuma comanda fechada hoje ainda.</p>'
        : fechadas.map(c => `
          <div class="comanda-movimentacao">
            <div class="comanda-movimentacao__cabecalho">
              <span class="comanda-movimentacao__mesa">${escaparHtmlAdmin(c.mesa_cliente)} · #${c.numero_comanda}</span>
              <span style="font-size:0.78rem;color:var(--admin-cor-texto-claro);">Atendida por ${escaparHtmlAdmin(c.funcionario_nome || '-')}</span>
            </div>
            <div class="comanda-movimentacao__rodape">
              <span>Fechada às ${formatarHoraAdmin(c.fechada_em)} · ${formasPagamentoLegendaAdmin[c.forma_pagamento] || c.forma_pagamento}</span>
              <span class="comanda-movimentacao__total">R$ ${formatarMoedaAdmin(c.total)}</span>
              <span style="font-size:0.75rem;color:var(--admin-cor-texto-claro);">${c.pago_no_caixa ? `Recebido por ${escaparHtmlAdmin(c.fechada_por_funcionario_nome || '-')}` : 'Recebido pelo garçom'}</span>
            </div>
          </div>
        `).join('');
    }
...
function construirAcoesStatusPedido(pedido) {
  ...
  if (passo && temPermissao('mudar_status_pedidos')) {
    ...
    html += `<span style="font-size:0.85rem;color:var(--admin-cor-texto-claro);">🛵 Aguardando entregador aceitar (fila automática)</span>`;
  }
...
function renderizarListaMovimentacao(linhas, opcoes = {}) {
  ...
  return linhas.map((c, indice) => {
    ...
    const atendentePor = mostrarAtendente
      ? ` <span style="font-weight:600;color:var(--admin-cor-texto-claro);">— atendido por ${escaparHtmlAdmin(c.funcionario_nome || '-')}</span>`
      : '';
    ...
    // prova contra a pessoa errada numa duvida de fechamento.
    const tagPagoNoCaixa = c.pago_no_caixa
      ? `<span style="display:inline-block;margin-top:4px;padding:2px 8px;border-radius:999px;background:#fef3c7;color:#92400e;font-size:0.72rem;font-weight:600;">💰 Recebido no caixa por ${escaparHtmlAdmin(c.fechada_por_funcionario_nome || '-')}</span>`
      : '';
