...
let ULTIMA_EQUIPE_OPERACIONAL = null; // cache da ultima resposta de /equipe, reusado ao abrir a Central de Entregas sem precisar recarregar

// =============================================
// VISTA "EQUIPE" (conteudo principal da aba) x "CADASTRO" (subpagina fixa)
// x "CENTRAL DE ENTREGAS" x "EQUIPE DE ENTREGADORES" (subpaginas fixas)
// =============================================
function ocultarTodasVistasFuncionarios() {
  ['funcionarios-vista-equipe', 'funcionarios-vista-cadastro', 'funcionarios-vista-entregas', 'funcionarios-vista-entregas-equipe']
    .forEach(id => document.getElementById(id)?.classList.add('oculto'));
}

// O menu lateral do painel (Aparencia, Produtos, Pedidos etc.) fica
// escondido nas telas da Central de Entregas -- com o layout de dashboard
// cheio de cards, o menu deixava tudo espremido. Um botao "hamburguer"
// fixo no canto permite abrir o menu de volta quando precisar navegar
// pra outra aba sem sair da Central de Entregas.
function configurarMenuLateralParaEntregas(ativo) {
  document.getElementById('painel-menu-lateral')?.classList.toggle('painel__menu--oculto', ativo);
  document.getElementById('botao-menu-lateral-toggle')?.classList.toggle('oculto', !ativo);
}

function mostrarVistaEquipe() {
  ocultarTodasVistasFuncionarios();
  document.getElementById('funcionarios-vista-equipe')?.classList.remove('oculto');
  configurarMenuLateralParaEntregas(false);
}
...
// Aplica no topo/fundo da Central de Entregas a mesma "Cor principal" que o
// lojista escolheu em Personalizacao (aba onde ele configura o cardapio do
// cliente) -- assim a central de entregas segue a identidade visual da loja
// em vez de uma cor fixa. Um leve escurecimento (overlay preto 15%) garante
// que o texto branco do topo continue legivel em qualquer cor escolhida.
function aplicarCorCentralEntregas() {
  const cor = ESTADO.estabelecimento?.cor_principal || '#0E7C3F';
  const painel = document.getElementById('funcionarios-vista-entregas');
  if (painel) painel.style.background = `linear-gradient(rgba(0,0,0,.15), rgba(0,0,0,.15)), ${cor}`;
}

async function mostrarVistaCentralEntregas() {
  ocultarTodasVistasFuncionarios();
  document.getElementById('funcionarios-vista-entregas')?.classList.remove('oculto');
  configurarMenuLateralParaEntregas(true);
  aplicarCorCentralEntregas();
  document.getElementById('central-entregas-nome-loja').textContent = ESTADO.estabelecimento?.nome || 'Minha loja';
  atualizarRelogioCentralEntregas();
  await Promise.all([carregarQrcodeCentralEntregas(), carregarDadosCentralEntregas()]);
}

// Painel numerico "Por KM" / "Valor Fixo" na Central de Entregas -- define
// a forma de pagamento e o valor pra TODOS os entregadores cadastrados de
// uma vez (sem precisar abrir a tela de Equipe pra editar um por um).
let MODO_VALOR_ENTREGA_GLOBAL = 'km';

function abrirPainelValorEntregaGlobal(modo) {
  MODO_VALOR_ENTREGA_GLOBAL = modo;
  const ehValorFixo = modo === 'entrega';
  document.getElementById('valor-entrega-global-titulo').textContent = modo === 'km' ? 'Valor por KM' : 'Valor fixo por entrega';
  document.getElementById('valor-entrega-global-descricao').textContent = ehValorFixo
    ? 'Aplica pra todos os entregadores cadastrados. Se também tiver um "Valor por KM" configurado, ele só entra na conta depois do limite de km abaixo.'
    : 'Aplica pra todos os entregadores cadastrados.';

  // Pre-preenche com o valor ja salvo (do primeiro entregador cadastrado
  // que tiver esse campo preenchido) em vez de sempre abrir vazio -- antes
  // isso parecia que o valor salvo tinha sumido, quando na verdade so nao
  // aparecia.
  const campoValor = modo === 'km' ? 'valor_por_km' : 'valor_por_entrega';
  const entregadorComValor = (ULTIMA_EQUIPE_OPERACIONAL?.entregadores || []).find(f => Number(f[campoValor]) > 0);
  const input = document.getElementById('valor-entrega-global-input');
  input.value = entregadorComValor ? Number(entregadorComValor[campoValor]).toFixed(2).replace('.', ',') : '';

  // O limite de km so faz sentido junto do valor fixo (e' o "ate quantos
  // km esse fixo cobre" antes do valor por km comecar a contar).
  document.getElementById('valor-fixo-limite-km-bloco').classList.toggle('oculto', !ehValorFixo);
  if (ehValorFixo) {
    const limiteInput = document.getElementById('valor-fixo-limite-km-input');
    const entregadorComLimite = (ULTIMA_EQUIPE_OPERACIONAL?.entregadores || []).find(f => Number(f.km_incluido_no_fixo) > 0);
    limiteInput.value = entregadorComLimite ? String(entregadorComLimite.km_incluido_no_fixo).replace('.', ',') : '';
    atualizarExemploLimiteKm();
  }

  document.getElementById('modal-valor-entrega-global').classList.remove('oculto');
  input.focus();
}

// Mostra um exemplo concreto enquanto o lojista digita o limite de km, pra
// confirmar visualmente que ficou do jeito que ele espera antes de salvar.
function atualizarExemploLimiteKm() {
  const exemploEl = document.getElementById('valor-fixo-limite-km-exemplo');
  const limite = Number((document.getElementById('valor-fixo-limite-km-input').value || '0').replace(',', '.')) || 0;
  const fixo = Number((document.getElementById('valor-entrega-global-input').value || '0').replace(',', '.')) || 0;
  if (limite <= 0) { exemploEl.textContent = 'Sem limite preenchido: o valor por km (se tiver) soma desde o km 1.'; return; }
  const entregadorComKm = (ULTIMA_EQUIPE_OPERACIONAL?.entregadores || []).find(f => Number(f.valor_por_km) > 0);
  const valorKm = entregadorComKm ? Number(entregadorComKm.valor_por_km) : 0;
  const exemploKm = limite + 2;
  const excedente = exemploKm - limite;
  const total = fixo + excedente * valorKm;
  exemploEl.textContent = valorKm > 0
    ? `Exemplo: uma entrega de ${exemploKm} km ficaria em R$ ${fixo.toFixed(2).replace('.', ',')} (fixo até ${limite}km) + ${excedente}km × R$ ${valorKm.toFixed(2).replace('.', ',')} = R$ ${total.toFixed(2).replace('.', ',')}.`
    : `Até ${limite} km vale só o fixo. Configure também um "Valor por KM" pra cobrar o excedente.`;
}
document.getElementById('valor-fixo-limite-km-input')?.addEventListener('input', (evento) => {
  evento.target.value = evento.target.value.replace(/[^0-9,]/g, '').replace(/(,.*),/g, '$1');
  atualizarExemploLimiteKm();
});

// Mascara de moeda (tipo "digitar de tras pra frente"): o usuario so digita
// numeros, e o campo vai se formatando sozinho como preco (ultimos 2
// digitos = centavos). Ex: digitar "5" -> "0,05"; "500" -> "5,00"; "50000"
// -> "500,00". Limite de 5 digitos (ate 999,99) -- usar em QUALQUER campo
// de valor em R$ do painel (nao so' nesse aqui).
...
document.getElementById('valor-entrega-global-input')?.addEventListener('input', aplicarMascaraMoeda);

async function salvarValorEntregaGlobal() {
  const valorDigitado = document.getElementById('valor-entrega-global-input').value;
  const valor = valorDigitado.replace(',', '.'); // formato brasileiro -> formato que o backend/SQL espera
  if (valorDigitado === '' || isNaN(Number(valor)) || Number(valor) < 0) { mostrarToast('Digite um valor válido.', true); return; }

  const campoValor = MODO_VALOR_ENTREGA_GLOBAL === 'km' ? 'valor_por_km' : 'valor_por_entrega';
  const dadosParaSalvar = { [campoValor]: valor };

  // O limite de km so existe no modal do Valor Fixo -- se o lojista deixou
  // em branco, manda 0 (sem limite, valor por km conta desde o km 1).
  if (MODO_VALOR_ENTREGA_GLOBAL === 'entrega') {
    const limiteDigitado = document.getElementById('valor-fixo-limite-km-input').value;
    dadosParaSalvar.km_incluido_no_fixo = limiteDigitado === '' ? '0' : limiteDigitado.replace(',', '.');
  }

  const botao = document.getElementById('botao-confirmar-valor-entrega-global');
  botao.disabled = true;
  botao.textContent = 'Salvando...';

  try {
    const equipe = ULTIMA_EQUIPE_OPERACIONAL || await apiListarEquipeOperacional();
    const entregadores = equipe.entregadores || [];

    // So atualiza o campo desse botao (Por KM ou Valor Fixo) -- o outro
    // campo do entregador fica intacto (atualizacao parcial no backend),
    // entao preencher os dois separadamente soma os dois na comissao.
    await Promise.all(entregadores.map(f => apiAtualizarFuncionario(f.id, dadosParaSalvar)));

    mostrarToast(`Valor salvo pra ${entregadores.length} entregador(es).`);
    document.getElementById('modal-valor-entrega-global').classList.add('oculto');
    carregarEquipeOperacional();
  } catch (erro) {
    mostrarToast(erro.message || 'Erro ao salvar o valor.', true);
  } finally {
    botao.disabled = false;
    botao.textContent = 'Salvar pra todos';
  }
}

async function mostrarVistaEquipeEntregas() {
  ocultarTodasVistasFuncionarios();
  document.getElementById('funcionarios-vista-entregas-equipe')?.classList.remove('oculto');
  configurarMenuLateralParaEntregas(true);
  atualizarDataHoraEquipeEntregadores();

  try {
    const [equipe, pedidosEmRota] = await Promise.all([
      ULTIMA_EQUIPE_OPERACIONAL ? Promise.resolve(ULTIMA_EQUIPE_OPERACIONAL) : apiListarEquipeOperacional(),
      apiListarPedidos('saiu_entrega')
    ]);
    ULTIMA_EQUIPE_OPERACIONAL = equipe;
    renderizarTabelaEquipeEntregadores(equipe.entregadores || [], pedidosEmRota);
  } catch (erro) {
    document.getElementById('equipe-entregadores-tabela-corpo').innerHTML = '<tr><td colspan="5" class="lista-vazia">Não foi possível carregar a equipe.</td></tr>';
  }
}

function atualizarDataHoraEquipeEntregadores() {
  const agora = new Date();
  document.getElementById('equipe-entregadores-data').textContent = agora.toLocaleDateString('pt-BR');
  document.getElementById('equipe-entregadores-hora').textContent = agora.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' });
}

function renderizarTabelaEquipeEntregadores(entregadores, pedidosEmRota) {
  const ativos = entregadores.filter(f => f.ativo);
  const emRota = ativos.filter(f => f.em_entrega);
  const disponiveis = ativos.filter(f => f.disponivel_entrega && !f.em_entrega);
  const indisponiveis = ativos.filter(f => !f.disponivel_entrega && !f.em_entrega);

  document.getElementById('equipe-stat-ativos').textContent = ativos.length;
  document.getElementById('equipe-stat-em-rota').textContent = emRota.length;
  document.getElementById('equipe-stat-disponivel').textContent = disponiveis.length;
  document.getElementById('equipe-stat-indisponivel').textContent = indisponiveis.length;
  document.getElementById('equipe-entregadores-total-rodape').textContent = `Total de entregadores cadastrados: ${entregadores.length}`;

  const corpo = document.getElementById('equipe-entregadores-tabela-corpo');
  if (entregadores.length === 0) {
    corpo.innerHTML = '<tr><td colspan="5" class="lista-vazia">Nenhum entregador cadastrado ainda.</td></tr>';
    return;
  }

  corpo.innerHTML = entregadores.map(f => {
    const pedidoAtual = f.em_entrega ? pedidosEmRota.find(p => p.entregador_id === f.id) : null;
    const nota = f.nota_media ? `<span class="equipe-entregadores-dash__nota">⭐ ${f.nota_media} <span class="equipe-entregadores-dash__nota-qtd">(${f.total_avaliacoes})</span></span>` : '';

    let statusHtml, localizacaoHtml, acoesHtml;
    if (!f.ativo) {
      statusHtml = `<span class="equipe-entregadores-dash__badge equipe-entregadores-dash__badge--indisponivel">🚫 Inativo</span>`;
      localizacaoHtml = '—';
      acoesHtml = '';
    } else if (pedidoAtual) {
      statusHtml = `<span class="equipe-entregadores-dash__badge equipe-entregadores-dash__badge--rota">🛵 Em rota</span>`;
      localizacaoHtml = `<div>${escaparHtmlAdmin(pedidoAtual.cliente_endereco || 'Endereço não informado')}</div><div class="equipe-entregadores-dash__sublinha">Entrega em andamento</div>`;
      acoesHtml = `<button type="button" class="botao-secundario" data-ver-detalhe-entregador="${f.id}" data-nome-entregador-tabela="${escaparHtmlAdmin(f.nome)}">🗺️ Ver rota</button>`;
    } else if (f.disponivel_entrega) {
      statusHtml = `<span class="equipe-entregadores-dash__badge equipe-entregadores-dash__badge--disponivel">⏸ Disponível</span>`;
      localizacaoHtml = `<span class="equipe-entregadores-dash__sublinha">Aguardando próximo pedido</span>`;
      acoesHtml = f.telefone
        ? `<a class="botao-secundario" href="https://wa.me/55${f.telefone.replace(/\D/g, '')}" target="_blank" rel="noopener">💬 Chamar</a>`
        : `<button type="button" class="botao-secundario" disabled title="Sem telefone cadastrado">💬 Chamar</button>`;
    } else {
      statusHtml = `<span class="equipe-entregadores-dash__badge equipe-entregadores-dash__badge--indisponivel">🚫 Indisponível</span>`;
      localizacaoHtml = `<span class="equipe-entregadores-dash__sublinha">Fora de serviço</span>`;
      acoesHtml = `<button type="button" class="botao-secundario" data-liberar-entregador="${f.id}">📅 Liberar</button>`;
    }

    return `
      <tr>
        <td>
          <div class="equipe-entregadores-dash__avatar-nome">
            <span class="equipe-entregadores-dash__avatar" style="background:${corAvatarEntregador(f.id)}">${(f.nome || '?').trim().charAt(0).toUpperCase()}</span>
            <div>
              <div class="equipe-entregadores-dash__nome">${escaparHtmlAdmin(f.nome)}</div>
              ${nota}
            </div>
          </div>
        </td>
        <td>${f.telefone ? `📞 ${escaparHtmlAdmin(f.telefone)}` : '—'}</td>
        <td>${statusHtml}</td>
        <td>${localizacaoHtml}</td>
        <td>
          <div style="display:flex;gap:6px;flex-wrap:wrap;">
            ${acoesHtml}
            <div class="equipe-entregadores-dash__menu">
              <button type="button" class="botao-secundario" data-abrir-menu-acoes-entregador="${f.id}">⋮</button>
              <div class="equipe-entregadores-dash__menu-opcoes oculto" id="menu-acoes-entregador-${f.id}">
                <button type="button" data-ver-valores-entregador="${f.id}" data-nome-entregador="${escaparHtmlAdmin(f.nome)}">💰 Histórico e valores</button>
                <button type="button" data-ver-link-acesso="${f.id}" data-token-acesso="${f.token_acesso || ''}" data-caminho-app="../entregador/index.html">🔗 Link de acesso</button>
              </div>
            </div>
          </div>
        </td>
  ...
  corpo.querySelectorAll('[data-ver-detalhe-entregador]').forEach(botao => {
    botao.addEventListener('click', () => abrirDetalheEntregador(botao.getAttribute('data-ver-detalhe-entregador'), botao.getAttribute('data-nome-entregador-tabela')));
  });

  corpo.querySelectorAll('[data-liberar-entregador]').forEach(botao => {
    botao.addEventListener('click', async () => {
      try {
        await apiAlternarDisponibilidadeEntregador(botao.getAttribute('data-liberar-entregador'), true);
        mostrarToast('Entregador liberado — voltou a ficar disponível.');
        mostrarVistaEquipeEntregas();
      } catch (erro) {
        mostrarToast(erro.message, true);
      }
    });
  ...
  corpo.querySelectorAll('[data-abrir-menu-acoes-entregador]').forEach(botao => {
    botao.addEventListener('click', (evento) => {
      evento.stopPropagation();
      const id = botao.getAttribute('data-abrir-menu-acoes-entregador');
      const menu = document.getElementById(`menu-acoes-entregador-${id}`);
      const jaAberto = !menu.classList.contains('oculto');
      corpo.querySelectorAll('.equipe-entregadores-dash__menu-opcoes').forEach(m => m.classList.add('oculto'));
      if (!jaAberto) menu.classList.remove('oculto');
    });
  });

  corpo.querySelectorAll('[data-ver-valores-entregador]').forEach(botao => {
    botao.addEventListener('click', () => abrirDetalheEntregador(botao.getAttribute('data-ver-valores-entregador'), botao.getAttribute('data-nome-entregador')));
  });

  corpo.querySelectorAll('[data-ver-link-acesso]').forEach(botao => {
    botao.addEventListener('click', () => abrirModalLinkAcesso(botao));
  });
...
function atualizarRelogioCentralEntregas() {
  const el = document.getElementById('central-entregas-data-hora');
  if (el) el.textContent = new Date().toLocaleString('pt-BR', { dateStyle: 'short', timeStyle: 'short' });
}

async function carregarQrcodeCentralEntregas() {
  try {
    const resultado = await apiObterQrcodeEntregador();
    document.getElementById('central-entregas-qrcode').src = resultado.qrcode_base64;
  } catch (erro) {
    // Silencioso: se falhar, o cartao so fica sem a imagem do QR Code.
  }
}

// Paleta pra colorir o avatar (iniciais) de cada entregador na lista de
// "Entregas em andamento" -- so uma cor de identificacao visual, escolhida
// de forma estavel a partir do id do entregador (mesmo entregador sempre
// cai na mesma cor entre atualizacoes).
const PALETA_AVATAR_ENTREGADOR = ['#2563eb', '#16a34a', '#ea580c', '#7c3aed', '#0891b2', '#dc2626'];
function corAvatarEntregador(chave) {
  let soma = 0;
  for (const c of String(chave || '')) soma += c.charCodeAt(0);
  return PALETA_AVATAR_ENTREGADOR[soma % PALETA_AVATAR_ENTREGADOR.length];
}

// Lista "Entregadores": uma linha por entregador disponivel (ativo), nao
// por pedido. Ponto verde = esta em rota agora (mostra o pedido/endereco
// dele); ponto cinza = disponivel mas parado, sem rota no momento. Clicar
// em qualquer linha abre o historico completo (rotas ja concluidas, valor
// a receber, gorjetas) desse entregador.
async function carregarDadosCentralEntregas() {
  const listaEl = document.getElementById('lista-entregas-em-andamento');
  try {
    const [equipe, pedidos] = await Promise.all([
      apiListarEquipeOperacional(),
      apiListarPedidos()
    ]);
    ULTIMA_EQUIPE_OPERACIONAL = equipe;
    const pedidosEntrega = pedidos.filter(p => p.tipo_pedido !== 'balcao');

    const hojeStr = new Date().toDateString();
    const ehHoje = (iso) => iso && new Date(iso).toDateString() === hojeStr;

    const emRotaPedidos = pedidosEntrega.filter(p => p.status_pedido === 'saiu_entrega');
    const aguardandoColeta = pedidosEntrega.filter(p => p.status_pedido === 'pronto');
    const finalizadasHoje = pedidosEntrega.filter(p => p.status_pedido === 'entregue' && ehHoje(p.atualizado_em));
    const entregasHoje = pedidosEntrega.filter(p => ehHoje(p.criado_em)).length;

    document.getElementById('stat-entregas-hoje').textContent = entregasHoje;
    document.getElementById('stat-em-andamento').textContent = emRotaPedidos.length;
    document.getElementById('stat-finalizadas').textContent = finalizadasHoje.length;
    document.getElementById('stat-pendentes').textContent = aguardandoColeta.length;

    const entregadoresAtivos = (equipe.entregadores || []).filter(f => f.ativo);
    if (entregadoresAtivos.length === 0) {
      listaEl.innerHTML = '<div class="lista-vazia">Nenhum entregador cadastrado ainda.</div>';
      return;
    }

    listaEl.innerHTML = entregadoresAtivos.map(f => {
      const iniciais = (f.nome || '?').trim().charAt(0).toUpperCase();
      const pedidoAtual = f.em_entrega ? emRotaPedidos.find(p => p.entregador_id === f.id) : null;

      if (pedidoAtual) {
        const minutosFora = Math.max(0, Math.round((Date.now() - new Date(pedidoAtual.horario_saiu_entrega || pedidoAtual.atualizado_em).getTime()) / 60000));
        return `
          <div class="entrega-linha entrega-linha--clicavel" data-abrir-detalhe-entregador="${f.id}" data-nome-entregador-linha="${escaparHtmlAdmin(f.nome)}" role="button" tabindex="0">
            <div class="entrega-linha__avatar" style="background:${corAvatarEntregador(f.id)}">${iniciais}</div>
            <div class="entrega-linha__nome-bloco">
              <div class="entrega-linha__nome">${escaparHtmlAdmin(f.nome)}</div>
              <div class="entrega-linha__status-texto"><span class="entrega-linha__status-dot entrega-linha__status-dot--rota"></span>Em rota</div>
            </div>
            <div class="entrega-linha__info">
              <div class="entrega-linha__rota-titulo">#${pedidoAtual.numero_pedido || pedidoAtual.id.slice(0, 6)} · ${escaparHtmlAdmin(pedidoAtual.cliente_nome)}</div>
              <div class="entrega-linha__rota-sub">${escaparHtmlAdmin(pedidoAtual.cliente_endereco || 'Endereço não informado')} · saiu há ${minutosFora} min</div>
            </div>
            <span class="entrega-linha__badge entrega-linha__badge--rota">Em rota</span>
            <span class="entrega-linha__seta">›</span>
          </div>
        `;
      }
      ...
      return `
        <div class="entrega-linha entrega-linha--clicavel" data-abrir-detalhe-entregador="${f.id}" data-nome-entregador-linha="${escaparHtmlAdmin(f.nome)}" role="button" tabindex="0">
          ...
          <div class="entrega-linha__nome-bloco">
            <div class="entrega-linha__nome">${escaparHtmlAdmin(f.nome)}</div>
            <div class="entrega-linha__status-texto"><span class="entrega-linha__status-dot entrega-linha__status-dot--disponivel"></span>${f.disponivel_entrega ? 'Disponível' : 'Indisponível'}</div>
          </div>
          <div class="entrega-linha__info"></div>
          <span class="entrega-linha__badge entrega-linha__badge--aguardando">${f.disponivel_entrega ? 'Aguardando' : 'Pausado'}</span>
          <span class="entrega-linha__seta">›</span>
        </div>
      `;
    }).join('');

    listaEl.querySelectorAll('[data-abrir-detalhe-entregador]').forEach(linha => {
      linha.addEventListener('click', () => {
        abrirDetalheEntregador(linha.getAttribute('data-abrir-detalhe-entregador'), linha.getAttribute('data-nome-entregador-linha'));
      });
    });
  } catch (erro) {
    listaEl.innerHTML = '<div class="lista-vazia">Não foi possível carregar as entregas.</div>';
  }
}
...
let ENTREGADOR_ID_DETALHE_ABERTO = null;

// Abre a janela com tudo daquele entregador: historico de rotas (plantoes),
// valor a receber (comissao + gorjeta dos plantoes ainda nao pagos) e
// botao pra marcar como pago depois de acertar por fora do sistema.
async function abrirDetalheEntregador(id, nomeConhecido = null) {
  ENTREGADOR_ID_DETALHE_ABERTO = id;
  const funcionario = ULTIMA_EQUIPE_OPERACIONAL?.entregadores?.find(f => f.id === id);
  document.getElementById('detalhe-entregador-nome').textContent = funcionario?.nome || nomeConhecido || 'Entregador';
  document.getElementById('detalhe-entregador-situacao').textContent = funcionario
    ? [
        Number(funcionario.valor_por_entrega) > 0
          ? `Fixo: R$ ${formatarMoedaAdmin(funcionario.valor_por_entrega)}${Number(funcionario.km_incluido_no_fixo) > 0 ? ` (até ${funcionario.km_incluido_no_fixo}km)` : ''}`
          : null,
        Number(funcionario.valor_por_km) > 0 ? `R$ ${formatarMoedaAdmin(funcionario.valor_por_km)}/km${Number(funcionario.km_incluido_no_fixo) > 0 ? ' excedente' : ''}` : null
      ].filter(Boolean).join(' + ') || 'Nenhum valor de comissão configurado ainda'
    : '';

  document.getElementById('modal-detalhe-entregador').classList.remove('oculto');
  document.getElementById('detalhe-entregador-historico').innerHTML = '<div class="lista-vazia">Carregando...</div>';

  // Historico de plantoes (login/logout no app, botao "marcar pago") roda
  // separado das rotas/caixinha detalhadas -- um erro num nao deve travar
  // os outros.
  apiHistoricoPlantoesEntregador(id).then(renderizarDetalheEntregador).catch(() => {
    document.getElementById('detalhe-entregador-historico').innerHTML = '<div class="lista-vazia">Não foi possível carregar o histórico de plantões.</div>';
  });

  ESTADO_PEDIDOS_ENTREGADOR_MODAL = { periodo: 'hoje', itens: [], cursor: null, temMais: false };
  ESTADO_CAIXINHAS_ENTREGADOR_MODAL = { periodo: 'hoje', itens: [], cursor: null, temMais: false };
  carregarPedidosDetalhadosEntregador(true);
  carregarCaixinhasEntregador(true);
}

// ---- Rotas realizadas (numero do pedido + valor da rota) no modal do admin ----
// Mesmo endpoint/formula do app do proprio entregador (buscarHistoricoEntregasCore
// no backend), so que filtrado admin-side por :id -- garante que o numero
// mostrado aqui bate com o que o entregador ve no celular dele.
let ESTADO_PEDIDOS_ENTREGADOR_MODAL = { periodo: 'hoje', itens: [], cursor: null, temMais: false };
let ESTADO_CAIXINHAS_ENTREGADOR_MODAL = { periodo: 'hoje', itens: [], cursor: null, temMais: false };

function chipsPeriodoAdminHtml(idAlvo, periodoAtivo) {
  return PERIODOS_FILTRO_ADMIN.map(p => `
    <button type="button" class="chip-periodo ${p.valor === periodoAtivo ? 'chip-periodo--ativo' : ''}" data-chip-periodo-admin="${idAlvo}" data-valor-periodo="${p.valor}">${p.rotulo}</button>
  `).join('');
}
const PERIODOS_FILTRO_ADMIN = [
...
async function carregarPedidosDetalhadosEntregador(resetar) {
  if (!ENTREGADOR_ID_DETALHE_ABERTO) return;
  const estado = ESTADO_PEDIDOS_ENTREGADOR_MODAL;
  document.getElementById('detalhe-entregador-pedidos-filtro').innerHTML = chipsPeriodoAdminHtml('pedidos', estado.periodo);
  const el = document.getElementById('detalhe-entregador-pedidos');
  if (resetar) el.innerHTML = '<div class="lista-vazia">Carregando...</div>';
  try {
    const params = { periodo: estado.periodo, limite: 5 };
    if (!resetar && estado.cursor) params.antes = estado.cursor;
    const dados = await apiEntregasDetalhadasAdmin(ENTREGADOR_ID_DETALHE_ABERTO, params);
    estado.itens = resetar ? dados.entregas : estado.itens.concat(dados.entregas);
    estado.cursor = dados.proximo_cursor;
    estado.temMais = dados.tem_mais;
    estado.totaisPeriodo = dados.totais_periodo;
    renderizarPedidosDetalhadosEntregador();
  } catch (erro) {
    el.innerHTML = `<div class="lista-vazia">${erro.message}</div>`;
  }
}

function renderizarPedidosDetalhadosEntregador() {
  const estado = ESTADO_PEDIDOS_ENTREGADOR_MODAL;
  const el = document.getElementById('detalhe-entregador-pedidos');
  const tp = estado.totaisPeriodo || {};
  const resumo = `<div class="detalhe-entregador__linha" style="border-bottom:1px dashed var(--admin-cor-borda,#e2e2e2);">
      <div class="detalhe-entregador__linha-data">${tp.quantidade ?? 0} rota(s) no período</div>
      <div class="detalhe-entregador__linha-valor">R$ ${formatarMoedaAdmin(tp.valor_total)}</div>
    </div>`;
  const lista = estado.itens.length === 0
    ? '<div class="lista-vazia">Nenhuma rota nesse período.</div>'
    : estado.itens.map(e => `
        <div class="detalhe-entregador__linha">
          <div>
            <div class="detalhe-entregador__linha-data">#${e.numero_pedido ?? '-'} · ${new Date(e.horario_entregue).toLocaleDateString('pt-BR')}</div>
            <div class="detalhe-entregador__linha-sub">${escaparHtmlAdmin(e.endereco?.completo || '-')}</div>
            ${e.valor_pedido !== null ? `<div class="detalhe-entregador__linha-sub">${e.troco_para !== null ? 'Valor do pedido a receber' : 'Valor do pedido'}: R$ ${formatarMoedaAdmin(e.valor_pedido)}${e.troco_para !== null ? ` (troco p/ R$ ${formatarMoedaAdmin(e.troco_para)})` : ''}</div>` : ''}
          </div>
          <div class="detalhe-entregador__linha-valor">R$ ${formatarMoedaAdmin(e.valor_rota)}</div>
        </div>
      `).join('');
  el.innerHTML = resumo + lista;
  document.getElementById('botao-carregar-mais-pedidos-entregador').classList.toggle('oculto', !estado.temMais);
}

async function carregarCaixinhasEntregador(resetar) {
  if (!ENTREGADOR_ID_DETALHE_ABERTO) return;
  const estado = ESTADO_CAIXINHAS_ENTREGADOR_MODAL;
  const el = document.getElementById('detalhe-entregador-caixinhas');
  if (resetar) el.innerHTML = '<div class="lista-vazia">Carregando...</div>';
  try {
    const params = { periodo: 'tudo', limite: 5 };
    if (!resetar && estado.cursor) params.antes = estado.cursor;
    const dados = await apiCaixinhasAdmin(ENTREGADOR_ID_DETALHE_ABERTO, params);
    estado.itens = resetar ? dados.caixinhas : estado.itens.concat(dados.caixinhas);
    estado.cursor = dados.proximo_cursor;
    estado.temMais = dados.tem_mais;
    renderizarCaixinhasEntregador();
  } catch (erro) {
    el.innerHTML = `<div class="lista-vazia">${erro.message}</div>`;
  }
}

function renderizarCaixinhasEntregador() {
  const estado = ESTADO_CAIXINHAS_ENTREGADOR_MODAL;
  const el = document.getElementById('detalhe-entregador-caixinhas');
  el.innerHTML = estado.itens.length === 0
    ? '<div class="lista-vazia">Nenhuma caixinha registrada.</div>'
    : estado.itens.map(c => `
        <div class="detalhe-entregador__linha">
          <div class="detalhe-entregador__linha-data">${new Date(c.horario_entregue).toLocaleDateString('pt-BR')} · ${new Date(c.horario_entregue).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}</div>
          <div class="detalhe-entregador__linha-valor">R$ ${formatarMoedaAdmin(c.valor)}</div>
        </div>
      `).join('');
  document.getElementById('botao-carregar-mais-caixinhas-entregador').classList.toggle('oculto', !estado.temMais);
}

document.getElementById('detalhe-entregador-pedidos-filtro').addEventListener('click', (evento) => {
  const chip = evento.target.closest('[data-chip-periodo-admin="pedidos"]');
  if (!chip) return;
  ESTADO_PEDIDOS_ENTREGADOR_MODAL.periodo = chip.getAttribute('data-valor-periodo');
  carregarPedidosDetalhadosEntregador(true);
});
document.getElementById('botao-carregar-mais-pedidos-entregador').addEventListener('click', () => carregarPedidosDetalhadosEntregador(false));
document.getElementById('botao-carregar-mais-caixinhas-entregador').addEventListener('click', () => carregarCaixinhasEntregador(false));

// Historico completo: TODOS os entregadores, TODOS os periodos, desde a
// primeira entrega da loja. Busca uma vez e filtra em JS (evita bater na
// API a cada letra digitada na busca).
let HISTORICO_COMPLETO_ENTREGAS_CACHE = [];

async function abrirHistoricoCompletoEntregas() {
  document.getElementById('modal-historico-completo-entregas').classList.remove('oculto');
  document.getElementById('historico-completo-contador').textContent = 'Carregando...';
  document.getElementById('historico-completo-lista').innerHTML = '<div class="lista-vazia">Carregando...</div>';
  document.getElementById('historico-completo-busca').value = '';

  try {
    HISTORICO_COMPLETO_ENTREGAS_CACHE = await apiListarHistoricoCompletoEntregas();
    renderizarHistoricoCompletoEntregas(HISTORICO_COMPLETO_ENTREGAS_CACHE);
  } catch (erro) {
    document.getElementById('historico-completo-contador').textContent = '';
    document.getElementById('historico-completo-lista').innerHTML = '<div class="lista-vazia">Não foi possível carregar o histórico.</div>';
  }
}

function renderizarHistoricoCompletoEntregas(lista) {
  document.getElementById('historico-completo-contador').textContent =
    `${lista.length} entrega${lista.length === 1 ? '' : 's'} encontrada${lista.length === 1 ? '' : 's'}`;

  const el = document.getElementById('historico-completo-lista');
  if (lista.length === 0) {
    el.innerHTML = '<div class="lista-vazia">Nenhuma entrega registrada ainda.</div>';
    return;
  }

  el.innerHTML = lista.map(p => {
    const dataHora = new Date(p.horario_entregue || p.criado_em);
    return `
      <div class="detalhe-entregador__linha">
        <div>
          <div class="detalhe-entregador__linha-data">#${p.numero_pedido || p.id.slice(0, 6)} · ${escaparHtmlAdmin(p.entregador_nome || 'Sem entregador')}</div>
          <div class="detalhe-entregador__linha-sub">${escaparHtmlAdmin(p.cliente_nome)} · ${escaparHtmlAdmin(p.cliente_endereco || 'Endereço não informado')}</div>
        </div>
        <div style="text-align:right;">
          <div class="detalhe-entregador__linha-data">${dataHora.toLocaleDateString('pt-BR')}</div>
          <div class="detalhe-entregador__linha-sub">${dataHora.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}${p.distancia_km ? ` · ${Number(p.distancia_km).toFixed(1)}km` : ''}</div>
        </div>
      </div>
    `;
  }).join('');
...
document.getElementById('historico-completo-busca')?.addEventListener('input', (evento) => {
  const termo = evento.target.value.trim().toLowerCase();
  if (!termo) { renderizarHistoricoCompletoEntregas(HISTORICO_COMPLETO_ENTREGAS_CACHE); return; }
  const filtrado = HISTORICO_COMPLETO_ENTREGAS_CACHE.filter(p =>
    [p.entregador_nome, p.cliente_nome, p.cliente_endereco, p.numero_pedido]
      .some(campo => (campo || '').toString().toLowerCase().includes(termo))
  );
  renderizarHistoricoCompletoEntregas(filtrado);
});

function renderizarDetalheEntregador(plantoes) {
  const pendentes = plantoes.filter(p => !p.pago);
  const valorAReceber = pendentes.reduce((soma, p) => soma + Number(p.valor_total || 0), 0);
  const gorjetasAReceber = pendentes.reduce((soma, p) => soma + Number(p.total_gorjetas || 0), 0);
  const totalEntregas = plantoes.reduce((soma, p) => soma + Number(p.total_entregas || 0), 0);

  document.getElementById('detalhe-entregador-a-receber').textContent = `R$ ${formatarMoedaAdmin(valorAReceber)}`;
  document.getElementById('detalhe-entregador-gorjetas').textContent = `R$ ${formatarMoedaAdmin(gorjetasAReceber)}`;
  document.getElementById('detalhe-entregador-total-entregas').textContent = totalEntregas;

  const botaoPagar = document.getElementById('botao-marcar-pago-entregador');
  botaoPagar.disabled = pendentes.length === 0;
  botaoPagar.textContent = pendentes.length === 0 ? '✅ Tudo em dia, nada pendente' : '💾 Marcar tudo como pago';

  const historicoEl = document.getElementById('detalhe-entregador-historico');
  if (plantoes.length === 0) {
    historicoEl.innerHTML = '<div class="lista-vazia">Nenhum plantão fechado ainda.</div>';
    return;
  }

  historicoEl.innerHTML = plantoes.map(p => {
    const inicio = new Date(p.inicio).toLocaleDateString('pt-BR');
    const km = Number(p.total_km || 0).toFixed(1);
    return `
      <div class="detalhe-entregador__linha">
        <div>
          <div class="detalhe-entregador__linha-data">${inicio}</div>
          <div class="detalhe-entregador__linha-sub">${p.total_entregas || 0} entrega(s) · ${km} km · gorjeta R$ ${formatarMoedaAdmin(p.total_gorjetas || 0)}</div>
        </div>
        <div style="text-align:right;">
          <div class="detalhe-entregador__linha-valor">R$ ${formatarMoedaAdmin(p.valor_total || 0)}</div>
          <span class="detalhe-entregador__badge-pago ${p.pago ? 'detalhe-entregador__badge-pago--pago' : 'detalhe-entregador__badge-pago--pendente'}">${p.pago ? 'Pago' : 'Pendente'}</span>
        </div>
      </div>
    `;
  }).join('');
...
document.getElementById('botao-marcar-pago-entregador')?.addEventListener('click', async () => {
  if (!ENTREGADOR_ID_DETALHE_ABERTO) return;
  try {
    const resultado = await apiMarcarPlantoesComoPago(ENTREGADOR_ID_DETALHE_ABERTO);
    mostrarToast(resultado.mensagem);
    const plantoes = await apiHistoricoPlantoesEntregador(ENTREGADOR_ID_DETALHE_ABERTO);
    renderizarDetalheEntregador(plantoes);
  } catch (erro) {
    mostrarToast(erro.message, true);
  }
});

// Cartao "Entregadores" da Equipe: so identificacao (nome + situacao),

// sem os controles de disponibilidade/comissao -- esses agora ficam na
// vista "Equipe de entregadores", aberta a partir da Central de Entregas.
// Cartao "Entregadores" da Equipe: so um resumo (nao lista cada nome --
// com 15+ entregadores cadastrados a lista ficaria enorme dentro desse
// cartao pequeno). O detalhe de cada um fica na Central de Entregas.
function renderizarIdentificacaoEntregadores(entregadores) {
  const lista = document.getElementById('lista-identificacao-entregadores');
  if (!lista) return;
  const total = entregadores?.length || 0;
  if (total === 0) {
    lista.innerHTML = '<div class="lista-vazia">Nenhum entregador cadastrado ainda.</div>';
    return;
  }
  const disponiveis = entregadores.filter(f => f.ativo && f.disponivel_entrega && !f.em_entrega).length;
  const emEntrega = entregadores.filter(f => f.em_entrega).length;
  lista.innerHTML = `
    <div class="item-admin">
      <div class="item-admin__info">
        <div class="item-admin__titulo">${total} entregador${total === 1 ? '' : 'es'} cadastrado${total === 1 ? '' : 's'}</div>
        <div class="item-admin__subtitulo">${disponiveis} disponível${disponiveis === 1 ? '' : 'eis'} agora · ${emEntrega} em entrega</div>
      </div>
    </div>
  `;
}
...
function renderizarEquipeOperacional(equipe) {
  ...
  renderizarIdentificacaoEntregadores(equipe.entregadores);
...
// Extraida pra funcao compartilhada -- usada tanto pela tabela nova de
// Equipe de entregadores quanto pelo card antigo (renderizarListaEquipe).
async function abrirModalLinkAcesso(botao) {
...
function configurarFuncionarios() {
  ...
  // Cartao "Entregadores" na Equipe -> abre a Central de Entregas
  const cartaoCentralEntregas = document.getElementById('cartao-abrir-central-entregas');
  cartaoCentralEntregas?.addEventListener('click', mostrarVistaCentralEntregas);
  cartaoCentralEntregas?.addEventListener('keydown', (evento) => {
    if (evento.key === 'Enter' || evento.key === ' ') { evento.preventDefault(); mostrarVistaCentralEntregas(); }
  });

  document.getElementById('botao-voltar-equipe-de-entregas')?.addEventListener('click', () => {
    mostrarVistaEquipe();
    carregarEquipeOperacional();
  });

  document.getElementById('botao-abrir-equipe-entregas')?.addEventListener('click', () => mostrarVistaEquipeEntregas());

  document.getElementById('botao-voltar-central-entregas')?.addEventListener('click', mostrarVistaCentralEntregas);

  document.querySelectorAll('[data-filtro-forma-pagamento]').forEach(botao => {
    botao.addEventListener('click', () => abrirPainelValorEntregaGlobal(botao.getAttribute('data-filtro-forma-pagamento')));
  });

  document.getElementById('botao-confirmar-valor-entrega-global')?.addEventListener('click', salvarValorEntregaGlobal);

  document.getElementById('botao-atualizar-entregas')?.addEventListener('click', carregarDadosCentralEntregas);
  document.getElementById('botao-historico-completo-entregas')?.addEventListener('click', abrirHistoricoCompletoEntregas);

  document.getElementById('botao-menu-lateral-toggle')?.addEventListener('click', () => {
    document.getElementById('painel-menu-lateral')?.classList.toggle('painel__menu--oculto');
  });

  // Botao de fallback pro checkin diario -- existe desde a criacao do app
  // do entregador: quando a camera do celular do entregador nao consegue
  // ler o QR (camera quebrada, app sem permissao etc.), o admin gera esse
  // codigo aqui, manda por WhatsApp/e-mail, e o entregador cola no campo
  // de checkin manual do app dele (mesmo efeito de escanear o QR).
  document.getElementById('botao-gerar-codigo-checkin')?.addEventListener('click', async () => {
    try {
      const resultado = await apiObterQrcodeEntregador();
      document.getElementById('imagem-qrcode-entregador').src = resultado.qrcode_base64;
      ...
      mostrarToast(erro.message || 'Erro ao gerar o código do dia.', true);
    }
  });
