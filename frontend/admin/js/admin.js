...
async function abrirResumoFuncionario(id) {
  try {
    const dados = await apiResumoFuncionario(id);
    document.querySelectorAll('.painel__menu-item[data-aba]').forEach(b => b.classList.remove('ativo'));
    document.querySelectorAll('.aba').forEach(a => a.classList.add('oculto'));
    document.getElementById('aba-resumo-funcionario').classList.remove('oculto');

    // Trocou de funcionario -> volta pra aba "Pedidos" (ou "Histórico" se
    // for cargo Caixa, ver renderizarResumoFuncionario) e limpa o
    // historico carregado (senao ficaria mostrando o historico de outra
    // pessoa por engano ate clicar de novo).
    ...
    intervaloAutoAtualizarResumo = setInterval(async () => {
      ...
      try {
        const atualizado = await apiResumoFuncionario(funcionarioIdResumoAberto);
        renderizarResumoFuncionario(atualizado);
...
document.getElementById('botao-atualizar-resumo')?.addEventListener('click', () => {
  if (funcionarioIdResumoAberto) abrirResumoFuncionario(funcionarioIdResumoAberto);
});
...
function renderizarResumoFuncionario(dados) {
  ...
  if (tipo === 'caixa') {
    ...
    listaPagamentos.innerHTML = (pagamentos_hoje || []).length === 0
      ? '<p class="aba__descricao">Nenhum pagamento recebido no caixa hoje ainda.</p>'
  ...
  if (gorjetasIndividuais.length === 0) {
    listaGorjetas.innerHTML = '<p class="aba__descricao">Nenhuma gorjeta ainda hoje.</p>';
  } else {
  ...
  if (!comandas_hoje || comandas_hoje.length === 0) {
    container.innerHTML = '<p class="aba__descricao">Nenhuma movimentação hoje ainda.</p>';
  } else {
