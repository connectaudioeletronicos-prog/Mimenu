...
async function abrirDetalheEntregador(id, nomeConhecido = null) {
  ...
  document.getElementById('detalhe-entregador-pedidos').innerHTML = '<div class="lista-vazia">Carregando...</div>';

  // Historico de plantoes (login/logout no app) roda separado do historico
  // de pedidos entregues -- um erro no primeiro nao deve travar o segundo,
  // que costuma ter dado real mesmo quando o entregador nunca usou o
  // controle de plantao (entregas registradas antes desse recurso existir).
  const [resultadoPlantoes, resultadoPedidos] = await Promise.allSettled([
    apiHistoricoPlantoesEntregador(id),
    apiListarPedidos('entregue')
  ]);

  if (resultadoPlantoes.status === 'fulfilled') {
    renderizarDetalheEntregador(resultadoPlantoes.value);
  } else {
  ...
  if (resultadoPedidos.status === 'fulfilled') {
    const pedidosDoEntregador = resultadoPedidos.value.filter(p => p.entregador_id === id);
    renderizarPedidosEntregador(pedidosDoEntregador);
  } else {
    document.getElementById('detalhe-entregador-pedidos').innerHTML = '<div class="lista-vazia">Não foi possível carregar as entregas.</div>';
  }
}
...
function renderizarPedidosEntregador(pedidos) {
  const el = document.getElementById('detalhe-entregador-pedidos');
  if (pedidos.length === 0) {
    el.innerHTML = '<div class="lista-vazia">Nenhuma entrega registrada ainda.</div>';
    return;
  }
  el.innerHTML = pedidos.slice(0, 20).map(p => `
    <div class="detalhe-entregador__linha">
      <div>
        <div class="detalhe-entregador__linha-data">${new Date(p.atualizado_em || p.criado_em).toLocaleDateString('pt-BR')} · #${p.numero_pedido || p.id.slice(0, 6)}</div>
        <div class="detalhe-entregador__linha-sub">${escaparHtmlAdmin(p.cliente_nome)}</div>
      </div>
      <div class="detalhe-entregador__linha-valor">R$ ${formatarMoedaAdmin(p.total || p.valor_total || 0)}</div>
    </div>
  `).join('');
}
