...
function renderizarResumoFuncionario(dados) {
  ...
  document.getElementById('card-vendas-do-dia').textContent = `${formatarMoedaAdmin(resumo.vendas_do_dia)}`;
  ...
  document.getElementById('card-ticket-medio').textContent = `${formatarMoedaAdmin(resumo.ticket_medio)}`;

  const totalCartao = fechamento_caixa.total_cartao_credito + fechamento_caixa.total_cartao_debito;
  document.getElementById('fechamento-total-recebido').textContent = `${formatarMoedaAdmin(fechamento_caixa.total_recebido)}`;
  document.getElementById('fechamento-dinheiro').textContent = `${formatarMoedaAdmin(fechamento_caixa.total_dinheiro)}`;
  document.getElementById('fechamento-cartao').textContent = `${formatarMoedaAdmin(totalCartao)}`;
  document.getElementById('fechamento-credito').textContent = `${formatarMoedaAdmin(fechamento_caixa.total_cartao_credito)}`;
  document.getElementById('fechamento-debito').textContent = `${formatarMoedaAdmin(fechamento_caixa.total_cartao_debito)}`;
  document.getElementById('fechamento-pix').textContent = `${formatarMoedaAdmin(fechamento_caixa.total_pix)}`;

  document.getElementById('resumo-cartao-credito-valor').textContent = `${formatarMoedaAdmin(fechamento_caixa.total_cartao_credito)}`;
  document.getElementById('resumo-cartao-credito-qtd').textContent = `${fechamento_caixa.transacoes_cartao_credito} transações`;
  document.getElementById('resumo-cartao-debito-valor').textContent = `${formatarMoedaAdmin(fechamento_caixa.total_cartao_debito)}`;
    ...
    container.innerHTML = comandas_hoje.map(c => `
      <div class="comanda-movimentacao">
        ...
        ${(c.rodadas || []).flatMap(r => Array.isArray(r.itens) ? r.itens : []).map(item => `
          <div class="comanda-movimentacao__linha-item"><span>${item.quantidade}x ${escaparHtmlAdmin(item.nome)}</span><span>${formatarMoedaAdmin(item.preco * item.quantidade)}</span></div>
        `).join('')}
        <div class="comanda-movimentacao__rodape">
          ...
          <span class="comanda-movimentacao__total">${formatarMoedaAdmin(c.total)}</span>
          ${c.status === 'fechada' ? `<button type="button" class="comanda-movimentacao__corrigir" data-corrigir-comanda="${c.id}" data-mesa="${escaparHtmlAdmin(c.mesa_cliente)}" data-subtotal="${c.subtotal}" data-gorjeta="${c.gorjeta}">✏️ Corrigir</button>` : ''}
        </div>
      </div>
    `).join('');
...
function configurarFuncionarios() {
  ...
  const autoMarcarMudarStatus = (idCargo, idGrupoPermissoes) => {
    document.getElementById(idCargo).addEventListener('change', function () {
      ...
      // Garcom precisa de "criar_pedidos" pra poder abrir/gerenciar comanda
      // de mesa -- sem isso, o cadastro fica sem nenhuma permissao marcada
      // e ele nao consegue nem abrir a primeira comanda.
      if (this.value === 'garcom') {
        const caixa = document.querySelector(`#${idGrupoPermissoes} input[value="criar_pedidos"]`);
        if (caixa) caixa.checked = true;
      }
    });
  };
  ...
  document.getElementById('botao-salvar-edicao-funcionario').addEventListener('click', async () => {
    ...
    try {
      await apiAtualizarFuncionario(id, {
        ...
        forma_pagamento_entrega: document.getElementById('edit-func-forma-pagamento-entrega').value,
        valor_por_entrega: document.getElementById('edit-func-valor-por-entrega').value,
        valor_por_km: document.getElementById('edit-func-valor-por-km').value
      });
      ESTADO.funcionarios = await apiListarFuncionarios();
      renderizarFuncionariosAdmin();
      fecharModaisAdmin();
...
function abrirModalEditarFuncionario(id) {
  ...
  document.getElementById('edit-func-forma-pagamento-entrega').value = f.forma_pagamento_entrega || 'entrega';
  document.getElementById('edit-func-valor-por-entrega').value = f.valor_por_entrega ?? 0;
  document.getElementById('edit-func-valor-por-km').value = f.valor_por_km ?? 0;
